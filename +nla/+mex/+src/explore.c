            ...
            for (col=0; col<n; col++)  {
                ...
                if (starting_row_index == stopping_row_index)
                    ...
                    for (current_row_index = starting_row_index;
                         ...
                         current_row_index++) {
                        mexPrintf(
                            "\t(%"FMT_SIZE_T"u,%"FMT_SIZE_T"u) = %g+%g i\n",
                            ir[current_row_index]+1,
                            col+1, pc[total].real, pc[total].imag);
                        total++;
                      }
                }
            }
...
analyze_int8(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        mxInt8 *p;
        if(mxIsComplex(array_ptr)) {
            pc = mxGetComplexInt8s(array_ptr);
...
analyze_uint8(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        mxUint8 *p;
        if(mxIsComplex(array_ptr)) {
            pc = mxGetComplexUint8s(array_ptr);
            total_num_of_elements = mxGetNumberOfElements(array_ptr);
...
analyze_uint16(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        mxUint16  *p;
        if(mxIsComplex(array_ptr)) {
            pc = mxGetComplexUint16s(array_ptr);
            total_num_of_elements = mxGetNumberOfElements(array_ptr);
...
analyze_uint32(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        mxUint32 *p;
        if(mxIsComplex(array_ptr)) {
            pc = mxGetComplexUint32s(array_ptr);
            total_num_of_elements = mxGetNumberOfElements(array_ptr);
            for (index=0; index<total_num_of_elements; index++)  {
                mexPrintf("\t");
                display_subscript(array_ptr, index);
...
analyze_uint64(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        mxUint64  *p;
        if(mxIsComplex(array_ptr)) {
            pc = mxGetComplexUint64s(array_ptr);
            total_num_of_elements = mxGetNumberOfElements(array_ptr);
...
analyze_single(const mxArray *array_ptr)
{
    mwSize total_num_of_elements, index;
        ...
        total_num_of_elements = mxGetNumberOfElements(array_ptr);
        if (mxIsComplex(array_ptr)) {
            pc = mxGetComplexSingles(array_ptr);
            for (index=0; index<total_num_of_elements; index++)  {
                mexPrintf("\t");
                display_subscript(array_ptr, index);
                mexPrintf(" = %g + %gi\n",(*pc).real,(*pc).imag);
...
/* LocalWords:  buf di ui gi Compat prhs
 */
