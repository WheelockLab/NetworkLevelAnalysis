    ...
    methods
        ...
        function merge(obj, other_objects)
            ...
            for object_index = 1:numel(other_objects)
                ...
                for method_index = 1:numel(obj.test_methods)
                    ...
                    if obj.(test_method)
                        (statistic_prefix).p_value_permutations.v = [(statistic_prefix).p_value_permutations.v,...
                            other_object.(test_method).p_value_permutations.v];
                        
                        test_specific_statistics = fieldnames((statistic_prefix).(obj.test_name));
                        ...
                        for statistic_index = 1:numel(test_specific_statistics)
                            ...
                            if contains(other_object.(test_method).(obj.test_name).(test_statistic), "_permutations")
                                obj.(test_method).(obj.test_name).(test_statistic).v = [...
                                    (statistic_prefix).(obj.test_name).(test_statistic).v,...
                                    other_object.(test_method).(obj.test_name).(test_statistic).v...
                                ];
                            end
                        end
                    end
        ...
        function concatenateResult(obj, other_object)
                ...
                for method_index = 1:numel(obj.test_methods)
                    ...
                    if other_object.(test_method)
                        obj = nla.net2.result.NetworkTestResult(other_object.test_options, other_object.(test_method).p_value.size,...
                            other_object.test_name, fieldnames(other_object.(test_method).(obj.test_name)));
                    end
                end
            end
            ...
            for method_index = 1:numel(obj.test_methods)
                ...
                test_specific_statistics = fieldnames(obj.(test_method).(obj.test_name));
                for statistic_index = 1:numel(test_specific_statistics)
                    test_statistic = test_specific_statistics{statistic_index};
                    if ~contains(other_object.(test_method).(obj.test_name).(test_statistic), "_permutations")
                        (statistic_prefix).(obj.test_name).(strcat(test_statistic, "_permutations")).v(:, obj.last_index + 1) =...
                            other_object.(test_method).(obj.test_name).(test_statistic).v;
                    end
                end
    ...
    methods (Access = private)
        ...
        function createTestSpecificResultsStorage(obj, number_of_networks, test_specific_statistics)
            ...
            for test_method_index = numel(obj.test_methods)
                test_method = obj.test_methods(test_method_index);
                if obj.(test_method)
                    obj.(test_method).(obj.test_name) = struct();
                    for statistic_index = 1:numel(test_specific_statistics)
                        test_statistic = test_specific_statistics(statistic_index);
                        obj.(test_method).(obj.test_name).(test_statistic) = TriMatrix(number_of_networks, TriMatrixDiag.KEEP_DIAGONAL);
                    end
                end
            end
        end
