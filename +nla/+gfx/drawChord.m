function drawChord(ax, ax_width, net_atlas, sig_mat, color_map, sig_type, chord_type, coeff_min, coeff_max, representative)
    ...
    import nla.TriMatrix nla.TriMatrixDiag nla.PlotType nla.gfx.SigType
    import nla.gfx.genArcSegment

    if ~exist('sig_type', 'var'), sig_type = SigType.INCREASING; end
    if ~exist('chord_type', 'var'), chord_type = PlotType.CHORD; end
    ...
    for n = 1:net_atlas.numNets()
        ...
        outer = genArcSegment([0, 0], [n_start_rad, n_end_rad], circle_radius, 50);
        inner = genArcSegment([0, 0], [n_start_rad, n_end_rad], circle_radius_inner, 50);
        ...
        text_pos = genArcSegment([0, 0], [n_start_rad, n_end_rad], text_radius, 3);
        text_pos = text_pos(2, :);
        ...
        if chord_type == PlotType.CHORD_EDGE && net_size_rads_arr(n) < 0.25 && strlength(name_disp) > 5
            ...
            if text_angle > pi/2 && text_angle < 3 * pi/2 
                ...
                text(text_pos(1), text_pos(2), name_disp, 'HorizontalAlignment', 'right',...
                    'Rotation', rad2deg(text_angle));
            else
                text(text_pos(1), text_pos(2), name_disp, 'HorizontalAlignment', 'left',...
                    'Rotation', rad2deg(text_angle));
            end
        else
            if text_pos(2) > 0
            ...
            text(text_pos(1), text_pos(2), name_disp, 'HorizontalAlignment', 'center',...
                'Rotation', rad2deg(text_angle));
        end
    ...
    if representative
        ...
        if sig_type == SigType.INCREASING
            [~, idx] = sort(sig_mat.v);
        elseif sig_type == SigType.DECREASING
    ...
    if chord_type == PlotType.CHORD_EDGE
    ...
    if chord_type == PlotType.CHORD_EDGE
        for n = 1:net_atlas.numNets()
            for r = 1:net_atlas.nets(n).numROIs()
                ...
                ROI_center = genArcSegment([0, 0], [ROI_center_rad, ROI_center_rad],...
                    (circle_radius_inner + chord_radius) / 2, 1);
    ...
    for idx_iter = 1:numel(sig_mat.v)
        i = idx(idx_iter);
        if ~isnan(sig_mat.v(i)) && (...
                (sig_type == SigType.INCREASING && sig_mat.v(i) > coeff_min) ||...
                (sig_type == SigType.DECREASING && sig_mat.v(i) < coeff_max) ||...
                (sig_type == SigType.ABS_INCREASING && abs(sig_mat.v(i)) > 0)...
            )
            %% color
            sig = sig_mat.v(i);
            np_color = nla.gfx.valToColor(sig, coeff_min, coeff_max, color_map);
            ...
            if chord_type == PlotType.CHORD
                ...
                np1_points = genArcSegment([0, 0], [np1_start_rad, np1_end_rad], chord_radius, 2);
                np1_start = np1_points(1,:);
                np1_end = np1_points(2,:);
                % for the second net in the pair
                np2_points = genArcSegment([0, 0], [np2_start_rad, np2_end_rad], chord_radius, 2);
                np2_start = np2_points(1,:);
                np2_end = np2_points(2,:);

                %% generate connecting circles between inner/outer points
                [inner_origin, inner_origin_rad, inner_radius] =...
                    nla.gfx.findCircleFromTwoTangents(np2_start, np1_end, np2_start_rad + pi, np1_end_rad);
                inner = nla.gfx.genArcSegmentHandlePoorlyDefined(inner_origin, inner_origin_rad, inner_radius,...
                    np1_end, np2_start, 50);

                [outer_origin, outer_origin_rad, outer_radius] =...
                    nla.gfx.findCircleFromTwoTangents(np2_end, np1_start, np2_end_rad + pi, np1_start_rad);
                outer = nla.gfx.genArcSegmentHandlePoorlyDefined(outer_origin, outer_origin_rad, outer_radius,...
                    np1_start, np2_end, 50);
                ...
                r1_center = genArcSegment([0, 0], [r1_center_rad, r1_center_rad], chord_radius + 1, 1);
                r2_center = genArcSegment([0, 0], [r2_center_rad, r2_center_rad], chord_radius + 1, 1);
                [arc_origin, arc_origin_rad, arc_radius] =...
                    nla.gfx.findCircleFromTwoTangents(r2_center, r1_center, r2_center_rad + pi, r1_center_rad);
                
                % Handle poorly-defined circles
                arc = nla.gfx.genArcSegmentHandlePoorlyDefined(arc_origin, arc_origin_rad, arc_radius,...
                    r1_center, r2_center, 50);
    ...
    viscircles(ax, [0, 0], circle_radius_inner - (space_between_nets_and_labels / 2), 'Color', 'w',...
        'LineWidth', space_between_nets_and_labels - 1);
