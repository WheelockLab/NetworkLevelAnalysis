classdef BrainPlot < handle
    ...
    methods
        ...
        function obj = BrainPlot(edge_test_result, edge_test_options, network_test_options, network1, network2, network_atlas)
            ...
            addParameter(brain_input_parser, "ROI_radius", 0, validNumberInput);
            addParameter(brain_input_parser, "surface_parcels", false, @isboolean);
            ...
            properties = {"edge_test_result", "edge_test_options", "network_test_options", "network1", "network2", "network_atlas", "ROI_radius", "surface_parcels"};
            ...
            figure_title = sprintf("Brain Visualization: Average of edge-level correlations between nets in [%s - %s] Network Pair",...
                obj.network_atlas.nets(obj.network1).name, obj.network_atlas.nets(obj.network2).name);
            ...
            obj.upper_limit = obj.edge_test_result.coeff_range(1);
            obj.lower_limit = obj.edge_test_result.coeff_range(2);
        ...
        function drawBrainPlots(obj)
            ...
            if obj.surface_parcels && ~islogical(obj.network_atlas.parcels)
                edges1 = obj.single_plot(subplot("Position", [0.45, 0.505, 0.53, 0.45], ViewPos.LAT, BrainColorMode.COLOR_ROIS, obj.color_map));
                edges2 = obj.single_plot(subplot("Position", [0.45, 0.055, 0.53, 0.45], ViewPos.MED, BrainColorMode.COLOR_ROIS, obj.color_map));
                all_edges = [edges1 edges2];
            else
                edges1 = obj.single_plot(subplot("Position", [0.45, 0.505, 0.26, 0.45], ViewPos.BACK, BrainColorMode.NONE));
            ...
            if obj.surface_parcels && ~islogical(obj.network_atlas.parcels)
                obj.single_plot(plot_axis, ViewPos.DORSAL, BrainColorMode.COLOR_ROIS, obj.colo_maps);
            else
                obj.single_plot(plot_axis, ViewPos.DORSAL, BrainColorMode.NONE);
            end
        ...
        function setROIandConnectivity(obj)
            ...
            for ROI_index1 = 1:numel(network1_ROI_indexes)
                network1_ROI_index = network1_ROI_indexes(ROI_index1);
                ...
                obj.ROI_values(ROI_index1) = (sum(coefficient1) + sum(coefficient2)) / (numel(coffecient1) + numel(coefficient2));
                obj.function_connectivity_values(ROI_index1) = (sum(function_connectivity1) + sum(function_connectivity2)) / (numel(function_connectivity1) + numel(function_connectivity2));
            end

            for ROI_index2 = 1:numel(network2_ROI_indexes)
                network2_ROI_index = network2_ROI_indexes(ROI_index2);
                ...
                ROI_value = (sum(coefficient1) + sum(coefficient2)) / (numel(coffecient1) + numel(coefficient2));
                ...
                if obj.network1 == obj.network2
                    ...
                    obj.function_connectivity_values(network2_ROI_index) = (obj.function_connectivity_values(network2_ROI_index) + function_connectivity_values) ./ 2;
                else
        ...
        function [coefficient1, coefficient2, function_connectivity1, function_connectivity2] =...
            ...
            if obj.is_noncorrelation_input
                ...
                coefficient1 = coefficient1(logical());
                coefficient2 = coefficient2(logical());
        ...
        function colors = mapColorsToLimits(obj)
            import nla.gfx.valToColor

            color_rows = size(colors);
            color_map_positive = obj.color_map(1:(color_rows/2), :);
            color_map_negative = obj.color_map(((color_rows/2) + 1):end, :);

            if functional_connectivity_exists
                colors_positive = valToColor(obj.function_connectivity_vals, -0.5, 0.5, color_map_positive);
                colors_negative = valToColor(obj.function_connectivity_vals, -0.5, 0.5, color_map_negative);
                colors(obj.ROI_values > 0, :) = colors_positive(obj.ROI_values > 0, :);
                colors(obj.ROI_values <= 0, :) = colors_negative(obj.ROI_values <= 0, :);
            else
                colors = valToColor(obj.ROI_values, obj.lower_limit, obj.upper_limit, obj.color_map);
            end
            colors(isnan(obj.ROI_values), :) = 0.5;
        end
        ...
        function edges = drawEdges(obj, ROI_position, plot_axis)
            ...
            for point1_index = 1:numel(point1_indexes)
                ...
                for point2_index = 1:numel(point2_indexes)
                    ...
                    if ~isempty(cofficient)
                        color_value = obj.mapValuesToLimits();
        ...
        function drawCortex(obj, anatomy, plot_axis, mesh_type, mesh_alpha, view_position, left_color, right_color)
            ...
            [left_mesh, right_mesh] = nla.gfx.anatToMesh(anatomy, mesh_type, view_position);

            % Set lighting and view positioning
            if view_position == ViewPos.LAT || ViewPos.MED
                view(plot_axis, [-90, 0]);
                ...
                switch view_position                      
                    case ViewPos.DORSAL
                        view(plot_axis, [0, 90]);
                        light(plot_axis, "Position", [100, 300, 100], "Style", "infiinite");
                    case ViewPos.LEFT
                        view(plot_axis, [-90, 0]);
            ...
            left_hemisphere = obj.drawCortexHemisphere(plot_axis, anatomy.hemi_l, left_mesh, left_color, mesh_alpha);
            right_hemisphere = obj.drawCortexHemisphere(plot_axis, anatomy.hemi_r, right_mesh, right_color, mesh_alpha);

            axis(plot_axis, "image");
            axis(plot_axis, "off");
        end

        function hemisphere = drawCortexHemisphere(obj, plot_axis, hemisphere_anatomy, mesh, color, mesh_alpha)
            properties = struct(
                "Faces", hemisphere_anatomy.elements(:, 1:3), "Vertices", mesh,...
                "EdgeColor", "none", "FaceColor", "interp", "FaceVertexCData", color,...
                "FaceLightin", "gourand", "FaceAlpha", mesh_alpha,...
                "AmbientStrength", 0.25, "DiffuseStrengh", 0.75, "SpecularStrength", 0.1...
            );
            hemisphere = patch(plot_axis, properties);
            hemisphere.Annotation.LegendInformation.IconDisplayStyle = "off";
        end

        function edges = singlePlot(obj, plot_axis, view_position, mesh_type, color_mode, color_matrix, upper_limit, lower_limit)
            ...
            [ROI_final_positions, ROI_colors = obj.getROIPositions(mesh_type, view_position, color_mode, color_matrix);
            if ~isequal(color_mode, nla.BrainColorMode.NONE) && obj.surface_parcels && ~islogical(obj.network_atlas.parcels) && isequal(size(obj.network_atlas.parcels.ctx_l,1), size(obj.network_atlas.anat.hemi_l.nodes, 1)) && isequal(size(obj.network_atlas.parcels.ctx_r, 1), size(obj.network_atlas.anat.hemi_r.nodes, 1))
                ROI_color_map = [0.5 0.5 0.5; ROI_colors];
                obj.drawCortex(obj.network_atlas.anat, plot_axis, mesh_type, mesh_alpha, view_position, ROI_color_map(obj.network_atlas.parcels.ctx_l + 1, :), ROI_color_map(obj.network_atlas.parcels.ctx_r + 1, :));
            else
                obj.drawCortex(obj.network_atlas.anat, plot_axis, mesh_type, mesh_alpha, view_position);
                if ~isequal(color_mode, nla.BrainColorMode.NONE)
                    for roi = 1:obj.network_atlas.numROIs():
                        nla.gfx.drawSphere(plot_axis, ROI_final_positions(roi, :), ROI_colors(roi, :), obj.ROI_radius);
                    end
                end
                edges = obj.drawEdges(ROI_final_positions, plot_axis);
            end
        ...
        function [ROI_final_positions, ROI_colors] = getROIPositions(obj, mesh_type, view_position, color_mode, color_matrix)
            import nla.gfx.BrainColorMode

            [left_mesh, right_mesh] = nla.gfx.anatToMesh(obj.network_atlas.anat, mesh_type, view_position);
            ...
            for network = 1:obj.network_atlas.numNets()
                for network_indexes = 1:numel(obj.network_atlas.nets(network).indexes)
                    ...
                    if left_distances(ROI_index) < right_distances(ROI_index)
                        offset = left_mesh(left_indexes(ROI_index, :) - obj.network_atlas.anat.hemi_l.nodes(ROI_index), :);
                    else:
                        offset = right_mesh(right_indexes(ROI_index, :) - obj.network_atlas.anat.hemi_r.nodes(ROI_index), :);
                    end
