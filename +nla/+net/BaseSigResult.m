classdef BaseSigResult < nla.net.BaseResult
    ...
    methods
        function obj = BaseSigResult(size)
            ...
            obj.observed_gt_expected = nla.TriMatrix(size, 'logical', nla.TriMatrixDiag.KEEP_DIAGONAL);
        end
            ...
            if obj.perm_count > 0
                if isfield(flags, 'show_full_conn') && flags.show_full_conn
                    if flags.plot_type == nla.PlotType.FIGURE
                        ...
                        obj.plotProb(edge_input_struct, input_struct, net_atlas, fig, 25, 425, obj.perm_prob_ew, false,...
                            sprintf('Full Connectome Method\nNetwork vs. Connectome Significance'), nla.net.mcc.None(),...
                            nla.Method.FULL_CONN, edge_result);
                    elseif flags.plot_type == nla.PlotType.CHORD || flags.plot_type == nla.PlotType.CHORD_EDGE
                        obj.plotChord(edge_input_struct, input_struct, net_atlas, obj.perm_prob_ew, false,...
                        sprintf('Full Connectome Method\nNetwork vs. Connectome Significance'), nla.net.mcc.None(),...
                        nla.Method.FULL_CONN, edge_result, flags.plot_type);
                    end
                end
                if isfield(flags, 'show_within_net_pair') && flags.show_within_net_pair
                    if flags.plot_type == nla.PlotType.FIGURE
                        ...
                        obj.plotProb(edge_input_struct, input_struct, net_atlas, fig, 0, 425, obj.within_np_prob, false,...
                            sprintf('Within Network Pair Method\nNetwork Pair vs. Permuted Network Pair'),...
                            input_struct.fdr_correction, nla.Method.WITHIN_NET_PAIR, edge_result);
                    elseif flags.plot_type == nla.PlotType.CHORD || flags.plot_type == nla.PlotType.CHORD_EDGE
                        obj.plotChord(edge_input_struct, input_struct, net_atlas, obj.within_np_prob, false,...
                            sprintf('Within Network Pair Method\nNetwork Pair vs. Permuted Network Pair'),...
                            input_struct.fdr_correction, nla.Method.WITHIN_NET_PAIR, edge_result, flags.plot_type);
                    end
                end
            end
        end
    ...
    methods (Access = protected)
        function genChordPlotFig(obj, edge_input_struct, input_struct, net_atlas, edge_result, plot_sig, plot_mat,...
            ...
            genChordPlotFig@nla.net.BaseResult(obj, edge_input_struct, input_struct, net_atlas, edge_result_thresh,...
                plot_sig, plot_mat, plot_max, cm, name_label, sig_increasing, chord_type)
        end
    end
end