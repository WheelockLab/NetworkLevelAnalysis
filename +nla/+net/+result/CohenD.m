classdef CohenD < nla.net.BasePermResult
    ...
    methods
        function obj = CohenD(size)
            ...
            % non-permuted stats
            obj.d = nla.TriMatrix(size, nla.TriMatrixDiag.KEEP_DIAGONAL);
            
            %% Within Net-Pair statistics (withinNP)
            obj.within_np_d = nla.TriMatrix(size, nla.TriMatrixDiag.KEEP_DIAGONAL);
        end
        ...
        function output(obj, edge_input_struct, input_struct, net_atlas, edge_result, flags)
            ...
            function brainFigsButtonClickedCallback(net1, net2)
                ...
                nla.gfx.drawBrainVis(edge_input_struct, input_struct, net_atlas, nla.gfx.MeshType.STD, 0.25, 3, true,...
            ...
            if obj.perm_count > 0
                if isfield(flags, 'show_full_conn') && flags.show_full_conn
                    d_sig = nla.TriMatrix(net_atlas.numNets(), 'logical', nla.TriMatrixDiag.KEEP_DIAGONAL);
                    d_sig.v = obj.d.v >= input_struct.d_max;
                    name_label = sprintf("Observed %s Full Connectome Significance\nD > %g", obj.name_formatted,...
                        input_struct.d_max);
                    
                    if flags.plot_type == nla.PlotType.FIGURE
                        %% Permuted probability (fullConn)
                        fig = nla.gfx.createFigure(500, 1000);

                        %% Check that network-pair size is not a confound
                        %obj.plotPermProbVsNetSize(net_atlas, subplot(2,1,2));
                        obj.plotValsVsNetSize(net_atlas, subplot(2,1,2), obj.d, "Full Connectome Observed Cohen's D vs. Net-Pair Size",...
                            "Cohen's D", "Cohen's D effect sizes");

                        %% Matrix plot
                        matrix_plot = nla.gfx.plots.MatrixPlot(fig, name_label, obj.d, net_atlas.nets, nla.gfx.FigSize.SMALL,...
                    ...
                    elseif flags.plot_type == nla.PlotType.CHORD || flags.plot_type == nla.PlotType.CHORD_EDGE
                        obj.genChordPlotFig(edge_input_struct, input_struct, net_atlas, edge_result, d_sig, obj.d,...
                            input_struct.d_max, [1,1,1;parula(256)], name_label, true, flags.plot_type);
                    end
                end
                
                if isfield(flags, 'show_within_net_pair') && flags.show_within_net_pair
                    within_np_d_sig = nla.TriMatrix(net_atlas.numNets(), 'logical', nla.TriMatrixDiag.KEEP_DIAGONAL);
                    ...
                    if flags.plot_type == nla.PlotType.FIGURE
                        ...
                        matrix_plot2 = nla.gfx.plots.MatrixPlot(fig, name_label, obj.within_np_d, net_atlas.nets,...
                            nla.gfx.FigSize.SMALL, 'network_clicked_callback', @brainFigsButtonClickedCallback,...
                            'marked_networks', within_np_d_sig, 'draw_legend', false, 'color_map', [1,1,1;parula(256)],...
                            'upper_limit', input_struct.d_max, 'lower_limit', 1);
                    ...
                    elseif flags.plot_type == nla.PlotType.CHORD || flags.plot_type == nla.PlotType.CHORD_EDGE
                        obj.genChordPlotFig(edge_input_struct, input_struct, net_atlas, edge_result, within_np_d_sig,...
                            obj.within_np_d, input_struct.d_max, [1,1,1;parula(256)], name_label, true, flags.plot_type);
                    end
                end
            end
        end
        ...
        function table_new = genSummaryTable(obj, table_old)
            table_new = [genSummaryTable@nla.net.BasePermResult(obj, table_old), table(obj.d.v,...
                'VariableNames', [obj.name])];
        end
    end
end