classdef HyperGeo < nla.net.BaseSigTest
    ...
    methods
        ...
        function result = run(obj, input_struct, edge_result, net_atlas, previous_result)
            ...
            observed_gt_expected = nla.TriMatrix(num_nets, 'logical', nla.TriMatrixDiag.KEEP_DIAGONAL);
            prob = nla.TriMatrix(num_nets, nla.TriMatrixDiag.KEEP_DIAGONAL);
            ...
            for row = 1:num_nets
                for col = 1:row
                    ...
                    prob.set(row, col, hygecdf(observed_sig, numel(edge_result.prob_sig.v), sum(edge_result.prob_sig.v),...
                        net_ROI_count, 'upper'));
                end
            end
            ...
            if previous_result ~= false
                result = obj.rank(net_atlas, previous_result, input_struct, @le, previous_result.prob,...
                    previous_result.prob, prob, prob, previous_result.prob, previous_result.prob, prob, prob);
            else
                result = nla.net.result.HyperGeo(num_nets);
