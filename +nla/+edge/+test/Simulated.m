classdef Simulated < nla.edge.BaseTest
    %SIMULATED Load previous simulated data
    properties (Constant)
        name = "Simulated data"
        coeff_name = "Simulated coeff"
    end
    
    methods
        function obj = Simulated()
        ...
        function result = run(obj, input_struct)
            ...
            result = nla.edge.result.Simulated(fcSquareEdgeSize, input_struct.prob_max);
    ...
    methods (Static)
        function inputs = requiredInputs()
            import nla.* % required due to matlab package system quirks
            inputs = {inputField.Number('prob_max', 'Edge-level P threshold <', 0, 0.05, 1), inputField.NetworkAtlasSimData()};
        end
    end
end
