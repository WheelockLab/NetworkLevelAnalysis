...
input_struct.precalc_obs_p = TriMatrix(obs_p_file.SIM_obs_p);

obs_coeff_file = load('examples/precalculated/SIM_obs_coeff.mat');
input_struct.precalc_obs_coeff = TriMatrix(obs_coeff_file.SIM_obs_coeff);

perm_p_file = load('examples/precalculated/SIM_perm_p.mat');
input_struct.precalc_perm_p = TriMatrix(net_atlas.numROIs - 1);
input_struct.precalc_perm_p.v = perm_p_file.SIM_perm_p;

perm_coeff_file = load('examples/precalculated/SIM_perm_coeff.mat');
input_struct.precalc_perm_coeff = TriMatrix(net_atlas.numROIs - 1);
